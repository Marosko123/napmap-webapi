...
func NewMongoService[DocType interface{}](config MongoServiceConfig) DbService[DocType] {
	...
	if svc.ServerHost == "" {
		svc.ServerHost = enviro("NAPMAP_API_MONGODB_HOST", "localhost")
	}

	if svc.ServerPort == 0 {
		port := enviro("NAPMAP_API_MONGODB_PORT", "27017")
	...
	if svc.UserName == "" {
		svc.UserName = enviro("NAPMAP_API_MONGODB_USERNAME", "")
	}

	if svc.Password == "" {
		svc.Password = enviro("NAPMAP_API_MONGODB_PASSWORD", "")
	}

	if svc.DbName == "" {
		svc.DbName = enviro("NAPMAP_API_MONGODB_DATABASE", "mb-napmap")
	}

	if svc.Collection == "" {
		svc.Collection = enviro("NAPMAP_API_MONGODB_COLLECTION", "stations")
	}

	if svc.Timeout == 0 {
		seconds := enviro("NAPMAP_API_MONGODB_TIMEOUT_SECONDS", "10")
